...
MARKET_RULES = {
    ...
    '交通・物流': [
        '運送','輸送','配送','物流','車両','自動車','バス','鉄道','航空','船舶','タクシー','運行','郵便','発送','引越','レンタカー','監視艇','トラック','散布車','除雪車','照明車','救難車','消防車','巡視船','練習船','配達','搬送','運搬','信書便','集配','ヘリコプター','救急車','新幹線','軌道敷設'
    ],
    '施設管理・清掃': [
    ...
    '化学・素材': [
        '塩化ナトリウム','凍結防止剤','液体窒素','原糸','鋼材','合金','樹脂','セメント','コンクリート','化学品','薬剤','原材料','火工品'
    ],
    '衣類・装備品': [
...
def classify(title):
    ...
    # If no specialist rule matched, a title that explicitly says purchase/buy/sale
    # is safely a goods procurement. Avoid plain substring matching so phrases such
    # as "purchase support" do not become goods by accident.
    if category == 'その他' and re.search(r'(?:購入|買入(?:れ)?|買い入れ|売買契約)(?:\s|[（(]|$|[0-9])', raw):
        category = '一般物品・備品'
