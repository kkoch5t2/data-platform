...
MARKET_RULES = {
    'IT・デジタル': [
        ...
        '電子計算機','アプリ','プログラム','ai','人工知能','通信回線','プリンタ','ファイアウォール','ハードウェア','wi-fi','wifi','スマートフォン','通信装置','無線装置','情報サービス','計算機資源','ポータル','電子化','ソフトウエア','sase','モバイルデバイス','共通基盤'
    ],
    '建設・土木': [
        '工事','建築','土木','舗装','改修','改築','新築','解体','施工','道路','橋梁','トンネル','河川',
        '港湾','測量','設計業務','営繕','耐震','電気設備','機械設備','空調設備'
    ],
    '医療・福祉': [
        '医療','病院','医薬','薬品','ワクチン','診療','検査薬','医療機器','福祉','介護','障害者',
        '健康診断','健診','看護','衛生材料','x線','麻酔','内視鏡','人工心肺','人工呼吸器','酸素濃縮器','ナースコール','臨床','手術支援','透視診断','点滴静注','髄注','注射','遺伝子組換'
    ],
    '研究・調査・コンサル': [
    ...
    '教育・研修': [
        '教育','研修','講習','教材','学校','大学','学習','訓練','セミナー','eラーニング','教科書'
    ],
    '交通・物流': [
        '運送','輸送','配送','物流','車両','自動車','バス','鉄道','航空','船舶','タクシー','運行','郵便','発送','引越','レンタカー','監視艇'
    ],
    '施設管理・清掃': [
        '清掃','警備','保守管理','施設管理','設備管理','ビル管理','点検','維持管理','受付業務',
        '廃棄物','ごみ処理','除草','剪定','保守業務','保守契約','修理'
    ],
    '広報・広告・制作': [
    ...
    'エネルギー・環境': [
        '電力','電気需給','電気の供給','使用する電気','ガス','燃料','石油','重油','灯油','軽油','ガソリン','再生可能エネルギー','環境','脱炭素','省エネ',
        '太陽光','蓄電池','水質','大気'
    ],
    '食品・給食': [
        '給食','食材','食品','弁当','飲料','米穀','野菜','肉類','魚介','食堂','精白米','玄米','備蓄食料'
    ],
    '科学・研究機器': [
        '測定装置','分析装置','実験装置','研究装置','顕微鏡','レーザー','検出器','シーケンサー','分光',
        '核磁気共鳴','量子デバイス','ビームライン','試薬','センサ','センサー','トランスデューサー','研究機器','実験機器'
    ],
    '出版・情報資料': [
        '書籍','雑誌','電子ジャーナル','図書','新聞','購読','六法','電子ブック','sciencedirect','wiley','springer','nature'
    ],
    '化学・素材': [
        '塩化ナトリウム','凍結防止剤','液体窒素','原糸','鋼材','合金','樹脂','セメント','コンクリート','化学品','薬剤','材料'
    ],
    '衣類・装備品': [
        'ジャケット','ブルゾン','ポロシャツ','シャツ','制服','被服','キャップ','帽子','防寒','レインジャケット','作業服','ズックぐつ','安全靴'
    ],
    '機械・設備': [
        '装置','機器','機械','設備','ポンプ','ボイラ','エアコン','空調機','冷凍機','発電機','変圧器','ロボット','整備','据付','クレーン','フォークリフト','圧縮機','製作','製造'
    ],
    '人材・業務委託': [
...
def classify(title):
    raw = unicodedata.normalize('NFKC', clean(title)).lower()
